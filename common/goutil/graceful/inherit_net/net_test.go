...
func TestInheritErrorOnListenUnixWithInvalidCount(t *testing.T) {
	var n inheritNet
	os.Setenv(envCountKey, "a")
	tmpdir, err := os.CreateTemp("", "TestInheritErrorOnListenUnixWithInvalidCount-")
	ensure.Nil(t, err)
	ensure.Nil(t, os.RemoveAll(tmpdir.Name()))
	_, err = n.Listen("unix", filepath.Join(tmpdir.Name(), "socket"))
	ensure.NotNil(t, err)
}
