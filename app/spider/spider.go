...
// UpsertItemField 为指定Rule动态追加结果字段名，并返回索引位置
// 已存在时返回原来索引位置
...
// GetEnableCookie 控制所有请求是否使用cookie
func (self *Spider) GetEnableCookie() bool {
	return self.EnableCookie
}

// SetPausetime 自定义暂停时间 pause[0]~(pause[0]+pause[1])，优先级高于外部传参
...
// DoHistory 返回是否作为新的失败请求被添加至队列尾部
...
// OutDefaultField 是否输出默认添加的字段 Url/ParentUrl/DownloadTime
