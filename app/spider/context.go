...
// UpsertItemField 为指定Rule动态追加结果字段名，并获取索引位置，
// 已存在时获取原来索引位置，
// 若ruleName为空，默认为当前规则。
...
// SetPausetime 自定义暂停区间(随机: Pausetime/2 ~ Pausetime*2)，优先级高于外部传参。
...
// GetItemField 由索引下标获取结果字段名，不存在时获取空字符串，
// 若ruleName为空，默认为当前规则。
...
// GetItemFieldIndex 由结果字段名获取索引下标，不存在时索引为-1，
// 若ruleName为空，默认为当前规则。
...
// GetUrl 从原始请求获取Url，从而保证请求前后的Url完全相等，且中文未被编码。
...
// GetCookie 获取响应的Cookie。
func (self *Context) GetCookie() string {
	return self.Response.Header.Get("Set-Cookie")
}

// GetDom GetHtmlParser returns goquery object binded to target crawl result.
...
// GetText GetBodyStr returns plain string crawled.
